...
#include <iomanip>
     ...
     * - cuisine_type: OTHER
     */
    Dish::Dish() : name_("UNKNOWN"), ingredients_(), prep_time_(0), price_(0.0), cuisine_type_(OTHER){}
    ...
    */
    ...
    std::string Dish::getCuisineType() const{
        ...
        else if (cuisine_type_ == MEXICAN){
            return "MEXICAN";
        }
        else if (cuisine_type_ == CHINESE){
            return "CHINESE";
        }
        else if (cuisine_type_ == INDIAN){
            return "INDIAN";
        }
        else if (cuisine_type_ == AMERICAN){
            return "AMERICAN";
        }
        else if (cuisine_type_ == FRENCH){
            return "FRENCH";
        }
        else{
            return "OTHER";
    ...
    void Dish::display() const{
        ...
        for (int i = 0; i < ingredients_.size(); i++){
            if ( i < (ingredients_.size()-1)) {
                ...
                std::cout << ingredients_[i] << std::endl;
            }
        }
